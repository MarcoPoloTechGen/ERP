...
alter table public.products add column if not exists project_id bigint references public.projects(id) on delete set null;
alter table public.products add column if not exists building_id bigint references public.project_buildings(id) on delete set null;
alter table public.products add column if not exists unit text;
alter table public.products add column if not exists unit_price numeric(12,2);
alter table public.products add column if not exists currency text not null default 'USD';
alter table public.invoices add column if not exists supplier_id bigint references public.suppliers(id) on delete set null;
alter table public.invoices add column if not exists project_id bigint references public.projects(id) on delete set null;
alter table public.invoices add column if not exists building_id bigint references public.project_buildings(id) on delete set null;
...
alter table public.projects add constraint projects_status_check check (status in ('active', 'completed', 'paused'));
...
alter table public.products add constraint products_currency_check check (currency in ('USD', 'IQD'));
alter table public.products drop constraint if exists products_unit_price_check;
alter table public.products add constraint products_unit_price_check check (unit_price is null or unit_price >= 0);

alter table public.invoices drop constraint if exists invoices_currency_check;
alter table public.invoices add constraint invoices_currency_check check (currency in ('USD', 'IQD'));
alter table public.invoices drop constraint if exists invoices_status_check;
alter table public.invoices add constraint invoices_status_check check (status in ('unpaid', 'partial', 'paid'));
...
alter table public.invoices add constraint invoices_paid_amount_check check (paid_amount >= 0 and paid_amount <= total_amount);
alter table public.invoices drop constraint if exists invoices_dates_check;
alter table public.invoices add constraint invoices_dates_check check (due_date is null or invoice_date is null or due_date >= invoice_date);

alter table public.worker_transactions drop constraint if exists worker_transactions_type_check;
alter table public.worker_transactions add constraint worker_transactions_type_check check (type in ('credit', 'debit'));
alter table public.worker_transactions drop constraint if exists worker_transactions_amount_check;
alter table public.worker_transactions add constraint worker_transactions_amount_check check (amount > 0);
alter table public.worker_transactions drop constraint if exists worker_transactions_currency_check;
alter table public.worker_transactions add constraint worker_transactions_currency_check check (currency in ('USD', 'IQD'));

alter table public.income_transactions drop constraint if exists income_transactions_amount_check;
alter table public.income_transactions add constraint income_transactions_amount_check check (amount > 0);
alter table public.income_transactions drop constraint if exists income_transactions_currency_check;
alter table public.income_transactions add constraint income_transactions_currency_check check (currency in ('USD', 'IQD'));
